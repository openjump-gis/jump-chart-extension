...
public class PieChart {
  ...
  public void addFeatures(FeatureCollection dataset) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      for (double datum : data) {
        totalValues = totalValues + datum;
        itemSum[i] = itemSum[i] + datum;
      }
  ...
  private Feature buildFeature(int index, double x, double y, double rad, double prop, double angle, double value, String name) {
    ...
    Vector<Coordinate> points = new Vector<>(10, 2);

    if (debug)
      System.out.println("Feature at:" + x + "," + y + " rad=" + rad + " prop=" + prop + "  angle=" + angle + "  value=" + value);
